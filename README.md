...
「範囲から提案」は、現時点ではFirestoreにある公開済み・検証済みコースカタログから、探索範囲・走行条件に合う候補をランキングします。任意エリアの道路を外部地図データから直接発掘する機能は、道路グラフと通行規制をバックエンドで検証できる同期基盤を追加してから提供します。未検証の外部ルーター結果を峠コースとして自動公開することはありません。
