...
export function RatingForm({ courseId, courseName, onCancel, onSave }: { courseId: string; courseName: string; onCancel: () => void; onSave: (rating: RatingSubmission) => Promise<void> }) {
  ...
  return (
    <div className="modal-backdrop" {...sheet.backdropProps}><form className={`modal rating-form ${sheet.className}`} style={sheet.style} onSubmit={submit} aria-labelledby="rating-title" {...sheet.dragProps} data-sheet-scroll {...sheet.scrollProps}>
