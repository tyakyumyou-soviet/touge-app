...
export function RatingForm({ courseId, courseName, onCancel, onSave }: { courseId: string; courseName: string; onCancel: () => void; onSave: (rating: RatingSubmission) => Promise<void> }) {
  ...
  return (
    <div className="modal-backdrop"><form className={`modal rating-form ${sheet.className}`} style={sheet.style} onSubmit={submit} aria-labelledby="rating-title">
      <div className="mobile-sheet-drag-region" {...sheet.dragProps}><div className="mobile-sheet-handle" aria-hidden="true" /><header><div><p className="eyebrow">DRIVE REVIEW</p><h2 id="rating-title">{courseName}を評価</h2></div><button type="button" className="icon-button" onClick={onCancel} aria-label="閉じる">×</button></header>
