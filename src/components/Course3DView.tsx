...
function projectPoint([x, y, z]: Point3, yaw: number, pitch: number): Point2 {
  const yawRad = (yaw * Math.PI) / 180
  const pitchRad = (pitch * Math.PI) / 180
  const rotatedX = x * Math.cos(yawRad) - y * Math.sin(yawRad)
  const depth = x * Math.sin(yawRad) + y * Math.cos(yawRad)
  return [500 + rotatedX, 365 + depth * Math.sin(pitchRad) - z * Math.cos(pitchRad)]
}
...
export function Course3DView({ course, onClose }: { course: Course; onClose: () => void }) {
  ...
  const modelPolygons = useMemo(() => model.left.slice(1).map((_, index) => {
    const i = index + 1
    const top = [model.left[i - 1], model.right[i - 1], model.right[i], model.left[i]].map((point) => projectPoint(point, modelYaw, modelPitch))
    const side = [model.right[i - 1], model.right[i], [model.right[i][0], model.right[i][1], model.right[i][2] - model.thickness], [model.right[i - 1][0], model.right[i - 1][1], model.right[i - 1][2] - model.thickness]].map((point) => projectPoint(point as Point3, modelYaw, modelPitch))
    return { top: pointsText(top), side: pointsText(side), color: `hsl(${148 - (i / Math.max(1, model.left.length - 1)) * 138} 66% 60%)` }
  }), [model, modelPitch, modelYaw])
  const modelStart = projectPoint(model.centers[0], modelYaw, modelPitch)
  const modelEnd = projectPoint(model.centers.at(-1)!, modelYaw, modelPitch)
  const modelCurrent = projectPoint(model.centers[Math.min(model.centers.length - 1, Math.round(progress * (model.centers.length - 1)))], modelYaw, modelPitch)
