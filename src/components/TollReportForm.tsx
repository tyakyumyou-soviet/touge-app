...
export function TollReportForm({ courseName, onCancel, onSave }: { courseName: string; onCancel: () => void; onSave: (report: TollReport) => Promise<void> }) {
  ...
  return <div className="modal-backdrop" {...sheet.backdropProps}><form className={`modal ${sheet.className}`} style={sheet.style} onSubmit={submit} aria-labelledby="toll-report-title" {...sheet.dragProps} data-sheet-scroll {...sheet.scrollProps}>
