  ...
  return <div className="modal-backdrop" role="presentation"><section data-map-occlusion="bottom-sheet" className={`modal course-form ${sheet.className} ${previewActive ? 'covered-by-detail' : ''} surface-${transitionState}`} style={sheet.style} aria-label="ルートビルダー">
    ...
    {stage === 'route' ? <div className="route-builder-stage">
      ...
      <div className="form-grid">
        ...
        {!editingCourse && <section className="wide course-share-picker audience-picker"><h3>公開するフレンド</h3><p>コースは選択したフレンドだけに表示されます。</p><label className="sharing-scope-select"><span>公開範囲</span><select value={audienceMode} onChange={(event) => setAudienceMode(event.target.value as 'all-friends' | 'lists')}><option value="all-friends">フレンド全員（{effectiveProfile?.followingIds?.length ?? 0}人）</option><option value="lists">フレンドリストから選ぶ</option></select></label>{audienceMode === 'lists' && <div className="audience-list-options">{(effectiveProfile?.friendLists ?? []).length ? <>{(effectiveProfile?.friendLists ?? []).map((list) => <label key={list.id}><input type="checkbox" checked={selectedFriendListIds.includes(list.id)} onChange={(event) => setSelectedFriendListIds((ids) => event.target.checked ? [...new Set([...ids, list.id])] : ids.filter((id) => id !== list.id))} /><span><strong>{list.name}</strong><small>{list.memberIds.length}人</small></span></label>)}<small>{selectedFriendListIds.length}件のリストを選択中</small></> : <p>フレンド画面でリストを作成すると選択できます。</p>}</div>}</section>}
