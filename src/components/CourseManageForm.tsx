...
export function CourseManageForm({ course, profile, onClose, onSave, onDelete }: Props) {
  ...
  return <div className="modal-backdrop" role="presentation"><section className={`modal course-manager ${sheet.className}`} style={sheet.style} role="dialog" aria-modal="true" aria-labelledby="course-manager-title">
    ...
    <form onSubmit={save}>
      ...
      <section className="course-manager-section"><h3>共有先とブラックリスト</h3><p>「フレンド・リンク限定」の場合に、共有するリストまたは相手を選べます。チェックした相手はこのコースを閲覧できません。</p>{(profile?.friendLists ?? []).map((list) => <label key={list.id} className="toggle-row"><input type="checkbox" checked={list.memberIds.length > 0 && list.memberIds.every((id) => (draft.allowedViewerIds ?? []).includes(id))} onChange={(event) => setDraft((value) => ({ ...value, allowedViewerIds: event.target.checked ? [...new Set([...(value.allowedViewerIds ?? []), ...list.memberIds])] : (value.allowedViewerIds ?? []).filter((id) => !list.memberIds.includes(id)), visibility: event.target.checked ? 'limited' : value.visibility }))} /> {list.name}に共有（{list.memberIds.length}人）</label>)}{(profile?.followingIds ?? []).map((id) => <label key={id} className="toggle-row"><input type="checkbox" checked={(draft.blockedViewerIds ?? []).includes(id)} onChange={(event) => setDraft((value) => ({ ...value, blockedViewerIds: event.target.checked ? [...new Set([...(value.blockedViewerIds ?? []), id])] : (value.blockedViewerIds ?? []).filter((item) => item !== id) }))} /> このコースを {id.slice(0, 8)} に非公開</label>)}</section>
