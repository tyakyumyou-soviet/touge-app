...
interface Props { user: User | null; course?: Course | null; courses: Course[]; focusCourseEditor?: boolean; onClose: () => void; onLogout?: () => void; onCreateJoined: (courses: Course[]) => Promise<void>; onUpdateCourse: (courseId: string, changes: EditableCourse) => Promise<void>; onDeleteCourse: (courseId: string) => Promise<void> }

export function CommunityPanel({ user, course, courses, focusCourseEditor = false, onClose, onLogout, onCreateJoined, onUpdateCourse, onDeleteCourse }: Props) {
  ...
  useEffect(() => {
    if (!focusCourseEditor || !courseDraft) return
    const timer = window.setTimeout(() => document.getElementById('course-editor')?.scrollIntoView({ block: 'start', behavior: 'smooth' }), 0)
    return () => window.clearTimeout(timer)
  }, [courseDraft, focusCourseEditor])
  useEffect(() => {
