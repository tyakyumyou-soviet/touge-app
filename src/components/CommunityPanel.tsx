      ...
      {!course && <>
        ...
        {settingsView === 'sharing' && <>
          ...
          <section className="presence-settings">
            ...
            <div className="presence-audience audience-picker" role="group" aria-labelledby="presence-audience-title">
              <h4 id="presence-audience-title">共有する相手</h4>
              <label className={`audience-option ${(profile?.locationSharing?.audience ?? 'friends') === 'friends' ? 'selected' : ''}`}><input type="radio" name="presence-audience" checked={(profile?.locationSharing?.audience ?? 'friends') === 'friends'} onChange={() => patchProfile({ locationSharing: { enabled: Boolean(profile?.locationSharing?.enabled), audience: 'friends', listIds: profile?.locationSharing?.listIds ?? [] } })} /><span><strong>フレンド全員</strong><small>{friendIds.length}人に共有</small></span></label>
              <label className={`audience-option ${profile?.locationSharing?.audience === 'lists' ? 'selected' : ''}`}><input type="radio" name="presence-audience" checked={profile?.locationSharing?.audience === 'lists'} onChange={() => patchProfile({ locationSharing: { enabled: Boolean(profile?.locationSharing?.enabled), audience: 'lists', listIds: profile?.locationSharing?.listIds ?? [] } })} /><span><strong>フレンドリストを選ぶ</strong><small>複数のリストを選択できます</small></span></label>
