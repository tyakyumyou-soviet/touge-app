...
export function CommunityPanel({ user, course, onClose, onLogout, onAdminOpen, onProfileSaved }: Props) {
  ...
  return <div className="modal-backdrop" role="presentation"><section className={`modal community-panel ${sheet.className}`} style={sheet.style} role="dialog" aria-modal="true" aria-labelledby="community-title" {...sheet.dragProps} data-sheet-scroll {...sheet.scrollProps}>
    <div className="mobile-sheet-drag-region"><div className="mobile-sheet-handle" aria-hidden="true" /><header><div><p className="eyebrow">COMMUNITY</p><h2 id="community-title">{course ? 'コメント・いいね' : viewTitle}</h2></div><div className="community-header-actions">{!course && settingsView !== 'home' && <button type="button" className="icon-button" onClick={() => setSettingsView('home')} aria-label="アカウントメニューへ戻る">←</button>}<button className="icon-button" onClick={onClose} aria-label="閉じる">×</button></div></header></div>
    {!user ? <div className="empty-state"><p>ログインするとプロフィール編集、フォロー、コメント、いいねが使えます。</p></div> : <>
      {!course && <>
        {settingsView === 'home' && <section className="account-menu"><div className="account-summary"><div className="profile-avatar" aria-label="プロフィール画像">{profileImageUrl ? <img src={profileImageUrl} alt="Googleアカウントのプロフィール画像" /> : (profile?.displayName || user.displayName || 'ド').slice(0, 1)}</div><div><strong>{profile?.displayName || user.displayName || 'ドライバー'}</strong><small>{profile?.vehicleName || '愛車を登録してプロフィールを整えよう'}</small></div></div><div className="account-menu-grid"><button type="button" onClick={() => setSettingsView('profile')}><strong>プロフィール</strong><small>自己紹介・愛車・SNS</small><span>→</span></button><button type="button" onClick={() => setSettingsView('friends')}><strong>フレンド</strong><small>フォロー・共有リスト</small><span>→</span></button><button type="button" onClick={() => setSettingsView('sharing')}><strong>共有・プライバシー</strong><small>地図・現在地・音楽</small><span>→</span></button><button type="button" onClick={() => setSettingsView('settings')}><strong>設定</strong><small>表示・パーソナライズ</small><span>→</span></button></div>{onAdminOpen && <button type="button" className="button secondary" onClick={onAdminOpen}>情報承認・編集</button>}{onLogout && <button type="button" className="text-button danger-button" onClick={onLogout}>ログアウト</button>}</section>}
        ...
        {settingsView === 'friends' && <><section className="friend-settings"><h3>フォロー中</h3>{(profile?.followingIds ?? []).length ? <div className="friend-members">{profile!.followingIds.map((id) => <span key={id}>{friendName(id)}</span>)}</div> : <p className="muted">コースのコメント画面から作成者をフォローできます。</p>}</section><section className="friend-settings"><h3>フレンドリスト</h3><p>コースや現在地の共有先を、リスト単位で選べます。</p><div className="inline-form"><input value={friendListName} onChange={(event) => setFriendListName(event.target.value)} placeholder="例: 伊豆ドライブ仲間" /><button type="button" className="button secondary" onClick={addFriendList}>リストを追加</button></div>{(profile?.friendLists ?? []).map((list) => <article key={list.id}><div className="friend-list-heading"><strong>{list.name}</strong><button type="button" className="text-button danger-button" onClick={() => patchProfile({ friendLists: (profile?.friendLists ?? []).filter((item) => item.id !== list.id) })}>削除</button></div>{(profile?.followingIds ?? []).length ? <div className="friend-members">{profile!.followingIds.map((id) => <label key={id}><input type="checkbox" checked={list.memberIds.includes(id)} onChange={() => toggleFriendInList(list.id, id)} /> {friendName(id)}</label>)}</div> : <small>先にコース作成者をフォローすると追加できます。</small>}</article>)}</section><button className="button primary" onClick={saveProfile} disabled={saving}>{saving ? '保存中…' : 'フレンド設定を保存'}</button></>}
        {settingsView === 'sharing' && <><section className="presence-settings"><h3>プロフィールを地図に表示</h3><select value={profile?.mapVisibility ?? 'friends'} onChange={(e) => patchProfile({ mapVisibility: e.target.value as UserProfile['mapVisibility'] })}><option value="all">全体に表示</option><option value="friends">フレンドのみ</option><option value="none">表示しない</option></select></section><section className="presence-settings"><h3>位置情報・再生中の音楽</h3><label className="toggle-row"><input type="checkbox" checked={Boolean(profile?.locationSharing?.enabled)} onChange={(event) => setLocationSharing(event.target.checked)} /> 位置情報を共有する</label><label>共有相手<select value={profile?.locationSharing?.audience ?? 'friends'} onChange={(event) => patchProfile({ locationSharing: { enabled: Boolean(profile?.locationSharing?.enabled), listIds: profile?.locationSharing?.listIds ?? [], audience: event.target.value as 'friends' | 'lists' } })}><option value="friends">フレンド全員</option><option value="lists">選択したリストのみ</option></select></label>{profile?.locationSharing?.audience === 'lists' && <div className="friend-members">{(profile.friendLists ?? []).map((list) => <label key={list.id}><input type="checkbox" checked={profile.locationSharing?.listIds.includes(list.id)} onChange={(event) => patchProfile({ locationSharing: { enabled: Boolean(profile.locationSharing?.enabled), audience: 'lists', listIds: event.target.checked ? [...new Set([...(profile.locationSharing?.listIds ?? []), list.id])] : (profile.locationSharing?.listIds ?? []).filter((id) => id !== list.id) } })} />{list.name}</label>)}</div>}<small>初期状態はオフです。オンの間のみ位置を更新します。</small><div className="inline-form"><input value={nowPlaying} onChange={(event) => setNowPlaying(event.target.value)} placeholder="再生中の曲名（任意）" /><button type="button" className="button secondary" onClick={() => void shareNowPlaying()}>曲名を共有</button></div>{presence.length > 0 && <div className="presence-list">{presence.map((item) => <span key={item.userId}>{item.displayName}{item.nowPlaying?.title ? ` · ♪ ${item.nowPlaying.title}` : ''}{item.location ? ' · 位置共有中' : ''}</span>)}</div>}</section><button className="button primary" onClick={saveProfile} disabled={saving}>{saving ? '保存中…' : '共有設定を保存'}</button></>}
